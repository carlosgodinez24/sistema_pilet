...
public class UsuarioFacade extends AbstractFacade<Usuario> implements UsuarioFacadeLocal {
    ...
    @Override
    public List<String> findAllPermByAcce(Object acce) {
        String query = "SELECT t.path FROM ( " +
            "SELECT u.acce_usua, GET_URL_PATH(p.codi_perm) AS path FROM permiso p " +
            "INNER JOIN permiso_rol pr ON p.codi_perm = pr.codi_perm " +
            "INNER JOIN rol r ON pr.codi_role = r.codi_role " +
            "INNER JOIN usuario_rol ur ON r.codi_role = ur.codi_role " +
            "INNER JOIN usuario u ON ur.codi_usua = u.codi_usua " +
            "WHERE p.esta_perm = 1 AND pr.esta_perm_role = 1 AND r.esta_role = 1 AND ur.esta_usua_role = 1 AND u.esta_usua = 1) T " +
            "WHERE T.acce_usua = ?1";
        Query q = getEntityManager().createNativeQuery(query);
        q.setParameter(1, acce);
        List resu = q.getResultList();
        return resu != null ? (List<String>)resu : new ArrayList<String>();
    }
